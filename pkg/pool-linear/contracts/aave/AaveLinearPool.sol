...
contract AaveLinearPool is LinearPool {
    ...
    {
        _lendingPool = IStaticAToken(address(wrappedToken)).POOL();
        _require(address(mainToken) == IStaticAToken(address(wrappedToken)).UNDERLYING_ASSET_ADDRESS(), Errors.TOKENS_MISMATCH);
    }
